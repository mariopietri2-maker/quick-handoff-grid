...
export function useDriverOrders(opts: { adminOverride?: boolean } = {}) {
  ...
  const offerTimeoutSec = platformSettings.dist_offer_timeout_seconds > 0
    ? platformSettings.dist_offer_timeout_seconds
    : 60;
  const assignmentMode = platformSettings.assignment_mode;
  const declinedRef = useRef<Record<string, number>>(loadDeclined());

  const fetchOrders = useCallback(async () => {
    if (!user) return;

    // assignment_mode + offer timeout come from shared usePlatformSettings cache
    ...
    const actives = (activeRows ?? []) as OrderWithItems[];
    setActiveDeliveries(actives);
    setActiveDelivery(actives[0] ?? null);
    const batchIds = [...new Set(actives.map((o) => (o as any).batch_id).filter(Boolean))];
    setCurrentBatchId(batchIds[0] ?? null);

    // Available offers: placed orders without a driver (or assigned to us in auto mode)
    // Admin override: see all placed orders for monitoring.
    ...
    if (!opts.adminOverride) {
      // Normal drivers only see unassigned or self-assigned offers
      offerQuery = offerQuery.or(`driver_id.is.null,driver_id.eq.${user.id}`);
    }

    const { data: offerRows } = await offerQuery;
    const allOffers = ((offerRows ?? []) as OrderWithItems[]).filter((o) => {
      const declinedAt = declinedRef.current[o.id];
      if (declinedAt && Date.now() - declinedAt < DECLINED_TTL_MS) return false;
      return true;
    });

    // Separate stacked (batch) offers from single offers
    const stacked = allOffers.filter((o) => (o as any).batch_id);
    const singles = allOffers.filter((o) => !(o as any).batch_id);
    setStackedOffers(stacked);
    setOffers(singles);

    setLoading(false);
  }, [user, opts.adminOverride]);

  useEffect(() => {
    fetchOrders();
  }, [fetchOrders]);

  // Realtime: new offers + status changes on our deliveries
  useEffect(() => {
    if (!user) return;
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          if (!row) return;
          // Relevant if: unassigned placed, assigned to us, or was assigned to us
          const relevant =
            row.status === 'placed' ||
