...
interface StoreOwnerProfilePanelProps {
  /** Prefer in-app tab switch over full navigation when embedded in StoreApp */
  onOpenSettings?: (storeId?: string) => void;
}

/**
 * Store owner account profile — used by /store/profile and the StoreApp Προφίλ tab.
 */
export function StoreOwnerProfilePanel({ onOpenSettings }: StoreOwnerProfilePanelProps) {
    ...
    <div className="max-w-lg mx-auto space-y-4 pb-6">
      ...
      <Card className="shadow-[var(--shadow-md)]">
        ...
        <CardContent className="space-y-3.5">
          <div>
            <Label htmlFor="store-owner-fullName">Ονοματεπώνυμο</Label>
            <Input
              id="store-owner-fullName"
              value={fullName}
          ...
          <div>
            <Label htmlFor="store-owner-phone">Τηλέφωνο επικοινωνίας</Label>
            <div className="relative">
              <Phone className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
              <Input
                id="store-owner-phone"
                className="pl-9"
                value={phone}
          ...
          <div>
            ...
            <Input id="store-owner-email" value={user?.email || ''} disabled className="bg-muted/50" />
      ...
      <Card className="shadow-[var(--shadow-md)]">
        ...
        <CardContent className="space-y-2">
          <p className="text-[12px] text-muted-foreground mb-1">
            Όνομα, διεύθυνση, ώρες και λειτουργία → ρυθμίσεις επιχείρησης.
          </p>
          {stores.length === 0 ? (
            ...
            stores.map((s) => (
              <button
                ...
                className="w-full flex items-center gap-3 rounded-xl border border-border bg-card p-3 hover:bg-muted/40 transition-colors text-left"
              >
