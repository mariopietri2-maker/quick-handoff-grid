...
export function OfferCard({ item, variant = 'default' }: { item: OfferItem; variant?: 'default' | 'wide' }) {
  ...
  const feeLabel = fee === 0 ? 'Δωρεάν' : `${fee.toFixed(2)}€`;
    ...
    >
      ...
      <div className={`relative ${variant === 'wide' ? 'aspect-[5/4]' : 'aspect-square'} bg-[hsl(var(--c-surface-muted))] overflow-hidden`}>
        ...
        <div className="absolute bottom-2 left-2 bg-[hsl(var(--c-surface)/0.95)] backdrop-blur rounded-full pl-1.5 pr-2 py-1 flex items-center gap-1 shadow-[0_1px_3px_hsl(0_0%_0%/0.12)]">
          ...
          <span className="text-[10px] font-extrabold text-[hsl(var(--c-text))]">{feeLabel}</span>
        </div>

        {/* Optional promo sticker */}
        {item.sticker && (
          <div className="absolute top-2 left-2 -rotate-6">
            <div className="bg-[hsl(var(--c-text))] text-[hsl(var(--c-bg))] text-[9px] font-black uppercase leading-tight px-2 py-1 rounded-md shadow-sm tracking-wide text-center">
              {item.sticker}
            </div>
          </div>
        )}
      ...
      <div className="p-2.5 space-y-1.5">
        <p className="text-[12.5px] font-bold text-[hsl(var(--c-text))] leading-tight line-clamp-2 min-h-[2.2em]">
          {item.name}
        </p>

        <div className="flex items-baseline gap-1.5">
          <span className="text-[15px] font-extrabold text-[hsl(var(--c-text))] tabular-nums">
        ...
        {/* Footer: store */}
        <div className="pt-1.5 mt-1 border-t border-[hsl(var(--c-border))] flex items-center gap-1.5">
          <div className="h-5 w-5 rounded-full bg-[hsl(var(--c-surface-muted))] flex items-center justify-center overflow-hidden shrink-0">
