...
export function ReviewList({ storeId }: ReviewListProps) {
  ...
  if (reviews.length === 0) {
    return (
      <p className="text-sm c-soft text-center py-4">Δεν υπάρχουν κριτικές ακόμα</p>
    );
  }
...
export function RatingBadge({ storeId }: { storeId: string }) {
  ...
  return (
    <span className="inline-flex items-center gap-1 text-[13px]">
      ...
      <span className="font-heading font-extrabold c-ink">{avg.toFixed(1)}</span>
      <span className="font-semibold c-soft">({count})</span>
    </span>
  );
}
