...
Deno.serve(async (req) => {
  ...
  // Defense in depth: verify_jwt=true already requires a signature-valid JWT at
  // the gateway layer. Accept only an exact service-role key (or CRON_SECRET)
  // so the caller must present the real service credential rather than a JWT
  // payload claiming role=service_role.
  const token = authHeader.slice('Bearer '.length).trim()
  const isService = Boolean(supabaseServiceKey) && token === supabaseServiceKey
  if (!isService) {
    return new Response(
    ...
    for (let i = 0; i < messages.length; i++) {
      ...
      try {
        await sendLovableEmail(
          ...
          // falls back to the default email API endpoint.
          // Set LOVABLE_SEND_URL / EMAIL_SEND_URL as a Supabase secret to override.
