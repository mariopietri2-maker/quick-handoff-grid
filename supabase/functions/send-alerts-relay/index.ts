...
Deno.serve(async (req) => {
  ...
  if (!title) {
    const m = text.match(/\] (.+)$/);
    title = m ? m[1].slice(0, 80) : "Fresh Delivery Alert";
  }
  ...
  const safeTitle = title.replace(/[^\x20-\x7E]/g, "").trim().slice(0, 110) || "Fresh Delivery Alert";
  ...
  try {
    const ntfyRes = await fetch(`https://ntfy.sh/${topic}`, {
      method: "POST",
      headers: {
        Title: `Fresh Delivery \u00b7 ${safeTitle}`,
        Priority: prio,
        Tags: tags,
      },
