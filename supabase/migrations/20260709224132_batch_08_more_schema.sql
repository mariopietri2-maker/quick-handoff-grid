...
DROP POLICY IF EXISTS "Support can view their announcements" ON public.announcements;
...
DROP POLICY IF EXISTS "Admins manage surge zones" ON public.surge_zones;
CREATE POLICY "Admins manage surge zones"
  ON public.surge_zones FOR ALL
  USING (public.has_role(auth.uid(), 'admin'))
  WITH CHECK (public.has_role(auth.uid(), 'admin'));

DROP POLICY IF EXISTS "Authenticated read active surge zones" ON public.surge_zones;
CREATE POLICY "Authenticated read active surge zones"
  ON public.surge_zones FOR SELECT
  USING (auth.uid() IS NOT NULL AND is_active = true);

DROP TRIGGER IF EXISTS update_surge_zones_updated_at ON public.surge_zones;
...
CREATE TABLE IF NOT EXISTS public.refunds (
  ...
  reason text,
  refund_type text NOT NULL DEFAULT 'wallet_credit',
  notes text,
...
DROP POLICY IF EXISTS "Drivers insert own offer events" ON public.driver_offer_events;
CREATE POLICY "Drivers insert own offer events"
  ON public.driver_offer_events FOR INSERT
  TO authenticated
  WITH CHECK (auth.uid() = driver_id);

DROP POLICY IF EXISTS "Drivers view own offer events" ON public.driver_offer_events;
CREATE POLICY "Drivers view own offer events"
  ON public.driver_offer_events FOR SELECT
  TO authenticated
  USING (auth.uid() = driver_id);

DROP POLICY IF EXISTS "Support and admins view all offer events" ON public.driver_offer_events;
...
DROP POLICY IF EXISTS "Support/admin manage wallets" ON public.customer_wallets;
...
DROP POLICY IF EXISTS "Customers view own ledger" ON public.customer_wallet_ledger;
CREATE POLICY "Customers view own ledger"
  ON public.customer_wallet_ledger FOR SELECT
  USING (auth.uid() = user_id OR is_support_or_admin(auth.uid()));

DROP POLICY IF EXISTS "Support/admin insert ledger" ON public.customer_wallet_ledger;
...
DROP POLICY IF EXISTS "Customers view own referrals" ON public.customer_referrals;
...
DROP POLICY IF EXISTS "Customers create own referral codes" ON public.customer_referrals;
CREATE POLICY "Customers create own referral codes"
  ON public.customer_referrals FOR INSERT
  WITH CHECK (auth.uid() = referrer_id);

DROP POLICY IF EXISTS "Support/admin update referrals" ON public.customer_referrals;
...
DROP POLICY IF EXISTS "Support/admin send notifications" ON public.driver_notifications;
...
DROP POLICY IF EXISTS "Support/admin view notifications" ON public.driver_notifications;
...
DROP POLICY IF EXISTS "Store owners view own wallet" ON public.store_wallets;
...
DROP POLICY IF EXISTS "Admins view all store wallets" ON public.store_wallets;
...
DROP TRIGGER IF EXISTS store_wallets_updated ON public.store_wallets;
...
DROP POLICY IF EXISTS "Store owners view own ledger" ON public.store_wallet_ledger;
...
DROP POLICY IF EXISTS "Admins manage store ledger" ON public.store_wallet_ledger;
CREATE POLICY "Admins manage store ledger"
  ON public.store_wallet_ledger FOR ALL
  USING (public.has_role(auth.uid(), 'admin'::app_role))
  WITH CHECK (public.has_role(auth.uid(), 'admin'::app_role));

CREATE INDEX IF NOT EXISTS idx_store_ledger_store ON public.store_wallet_ledger(store_id, created_at DESC);

-- ---------- ADMIN TREASURY (single platform money bag) ----------
CREATE TABLE IF NOT EXISTS public.admin_treasury (
  id integer PRIMARY KEY DEFAULT 1,
  admin_balance numeric NOT NULL DEFAULT 0,
  platform_pool numeric NOT NULL DEFAULT 0,
...
DROP POLICY IF EXISTS "Admins view treasury" ON public.admin_treasury;
CREATE POLICY "Admins view treasury"
  ON public.admin_treasury FOR SELECT
  USING (public.has_role(auth.uid(), 'admin'::app_role));

DROP POLICY IF EXISTS "Admins update treasury" ON public.admin_treasury;
...
DROP POLICY IF EXISTS "Admins manage treasury ledger" ON public.admin_treasury_ledger;
CREATE POLICY "Admins manage treasury ledger"
  ON public.admin_treasury_ledger FOR ALL
  USING (public.has_role(auth.uid(), 'admin'::app_role))
  WITH CHECK (public.has_role(auth.uid(), 'admin'::app_role));

CREATE INDEX IF NOT EXISTS idx_treasury_ledger_created ON public.admin_treasury_ledger(created_at DESC);
...
CREATE TABLE IF NOT EXISTS public.driver_cash_debts (
  ...
  driver_share numeric NOT NULL DEFAULT 0,
  store_share numeric NOT NULL DEFAULT 0,
...
DROP POLICY IF EXISTS "Drivers view own debts" ON public.driver_cash_debts;
CREATE POLICY "Drivers view own debts"
  ON public.driver_cash_debts FOR SELECT
  USING (auth.uid() = driver_id);

DROP POLICY IF EXISTS "Admins manage cash debts" ON public.driver_cash_debts;
CREATE POLICY "Admins manage cash debts"
  ON public.driver_cash_debts FOR ALL
  USING (public.has_role(auth.uid(), 'admin'::app_role))
  WITH CHECK (public.has_role(auth.uid(), 'admin'::app_role));

DROP POLICY IF EXISTS "Support view cash debts" ON public.driver_cash_debts;
CREATE POLICY "Support view cash debts"
  ON public.driver_cash_debts FOR SELECT
  USING (public.is_support_or_admin(auth.uid()));

CREATE INDEX IF NOT EXISTS idx_cash_debts_driver ON public.driver_cash_debts(driver_id, settled, created_at DESC);
...
-- Realtime
DO $$ BEGIN ALTER PUBLICATION supabase_realtime ADD TABLE public.store_wallets; EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN ALTER PUBLICATION supabase_realtime ADD TABLE public.admin_treasury; EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN ALTER PUBLICATION supabase_realtime ADD TABLE public.driver_cash_debts; EXCEPTION WHEN duplicate_object THEN NULL; END $$;
