/*
  Fair closest auto-dispatch:
  - Prefer closest drivers (distance-first scoring)
  - Soft €10/hour fairness so underpaid drivers get a boost
  - Keep offering forever: allow last-known GPS (up to 24h) so
    backgrounded "online" drivers are still eligible
  - Do not change quote_driver_payout — fairness only affects who gets offered
*/
...
SET
  ...
  dist_search_radius_km = GREATEST(COALESCE(dist_search_radius_km, 5), 15),
  target_hourly_eur = COALESCE(target_hourly_eur, 10)
WHERE id = 1;
...
DECLARE
  ...
  max_stack INT;
  near_km NUMERIC := 0.6;
  gps_fresh INTERVAL := INTERVAL '45 minutes';
  gps_stale_ok INTERVAL := INTERVAL '24 hours';
  target_h NUMERIC;
  w_dist NUMERIC;
  w_fair NUMERIC;
BEGIN
  ...
  WITH driver_pool AS (
    ...
    WHERE dp.is_active = true
      ...
      AND dl.updated_at > now() - gps_stale_ok
  ...
  scored AS (
    SELECT
      ...
      CASE
        WHEN dp.loc_at > now() - gps_fresh THEN 0::numeric
        ELSE LEAST(6::numeric, EXTRACT(EPOCH FROM (now() - dp.loc_at)) / 3600.0) * 0.8
  ...
  SELECT
    ...
    sc.v_type,
    ROUND(
      -- Lower score wins. Closest drivers first.
      (sc.dist_km * w_dist * 10)
      -- Under €target/h → negative boost (better rank). Over target → small penalty.
      + (
          CASE
