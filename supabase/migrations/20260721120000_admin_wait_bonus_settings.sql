/*
  Admin-configurable wait/delay bonus for drivers:
  - € per minute after a grace period
  - optional cap
  - applied into settle_order_commission as earnings.bonus
*/

ALTER TABLE public.platform_settings
  ADD COLUMN IF NOT EXISTS wait_bonus_rate_per_min numeric NOT NULL DEFAULT 0.10,
  ADD COLUMN IF NOT EXISTS wait_bonus_grace_minutes integer NOT NULL DEFAULT 10,
  ADD COLUMN IF NOT EXISTS wait_bonus_cap numeric NOT NULL DEFAULT 10;
...
SET
  ...
  wait_bonus_cap = COALESCE(wait_bonus_cap, 10)
WHERE id = 1;

-- Server-side wait bonus calc using the new settings columns
...
DECLARE
  ...
  rate_per_min numeric := 0.10;
  grace_minutes int := 10;
  cap_amount numeric := 10;
BEGIN
  ...
  SELECT
    COALESCE(wait_bonus_rate_per_min, 0.10),
    COALESCE(wait_bonus_grace_minutes, 10),
    COALESCE(wait_bonus_cap, 10)
  ...
  ELSE
    ...
    IF cap_amount IS NOT NULL AND cap_amount >= 0 AND computed_bonus > cap_amount THEN
      computed_bonus := cap_amount;
    END IF;
...
-- Expose wait-bonus knobs to the driver app (read-only public RPC)
...
-- Include wait-time bonus in driver settlement payout
