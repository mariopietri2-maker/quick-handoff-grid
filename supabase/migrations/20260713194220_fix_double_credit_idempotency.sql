/*
  Fix: Double driver wallet credit + double store wallet credit on order delivery.

  Root causes:
  1. settle_order_commission() was crediting store_wallets directly (no idempotency guard)
     AND trg_credit_store_wallet_on_delivery was ALSO crediting store_wallets.
     Result: every delivered order credited the store twice.

  2. settle_order_commission() was also crediting driver_wallets via ON CONFLICT DO UPDATE
     with no per-order guard. The commission_settled_at IS NOT NULL top-level guard should
     prevent this in normal flow, but any race condition or retry resulted in a double-credit
     since the driver_wallets upsert is not idempotent per-order.

  Fix:
  - Remove store_wallets credit from settle_order_commission (trg_credit_store_wallet_on_delivery
    already handles this correctly with a store_wallet_ledger idempotency guard).
  - Wrap driver_wallets INSERT in a WHERE NOT EXISTS guard keyed on wallet_transactions
    (earning_credit type), matching the pattern already used for that table.
*/
...
BEGIN
  ...
  -- store_keeps_amt is written to NEW.store_charge below.
  -- The store wallet credit is handled exclusively by trg_credit_store_wallet_on_delivery
  -- (an AFTER UPDATE trigger) which uses store_wallet_ledger as an idempotency guard.
  ...
  IF NEW.driver_id IS NOT NULL THEN
    ...
    IF driver_share_total > 0 THEN
      ...
      -- Guard: only credit driver_wallets if no earning_credit transaction exists for this order.
      -- Prevents double-credit on any retry/race.
