-- Closing the driver app must make them offline for admin + dispatch.
-- Tighten GPS freshness and clear stale "on shift" leftovers.

-- 1) Clear shift flags when location is missing or older than 3 minutes
UPDATE public.driver_state ds
SET
  shift_started_at = NULL,
  on_break = false,
  break_until = NULL,
  updated_at = now()
WHERE ds.shift_started_at IS NOT NULL
  AND NOT EXISTS (
    SELECT 1
    FROM public.driver_locations dl
    WHERE dl.driver_id = ds.driver_id
      AND dl.updated_at > now() - interval '3 minutes'
  );

-- 2) Drop stale GPS rows (> 3 min) so they stop looking "online"
DELETE FROM public.driver_locations
WHERE updated_at < now() - interval '3 minutes';

-- 3) Dispatch: only fresh GPS (3 min) + on-shift drivers — no 24h ghosts
