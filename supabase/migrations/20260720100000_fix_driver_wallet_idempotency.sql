/*
  Fix: Driver wallet can be double-credited on settle retries/races.

  wallet_transactions already has a per-order earning_credit guard, but
  driver_wallets was upserted unconditionally. Align both writes behind the
  same WHERE NOT EXISTS guard.
*/
...
BEGIN
  ...
  -- Store wallet credit is handled exclusively by trg_credit_store_wallet_on_delivery.
  ...
  IF NEW.driver_id IS NOT NULL THEN
    ...
    ELSE
      ...
      IF driver_share_total > 0 THEN
        -- Guard: only credit driver_wallets if no earning_credit exists for this order.
