package com.freshdelivery.nativedriver.ui.theme

import androidx.compose.ui.graphics.Color

/**
 * fresh2go — premium dark-first palette.
 * Deep slate surfaces with a vivid PAS Giannina blue action color that pops on dark.
 */
val FreshGreen = Color(0xFF2E6FE0)          // brand blue / primary action
val FreshGreenBright = Color(0xFF6FA8FF)    // vivid blue for text/icons on dark
val FreshGreenDark = Color(0xFF1B4BA0)
val FreshGreenDeep = Color(0xFF12336E)
val FreshGreenSoft = Color(0xFF14264A)      // dark blue container
val FreshGreenGlow = Color(0xFF8FC0FF)      // glow accent

val FreshBlack = Color(0xFFF0F4F1)          // primary text on dark surfaces
val FreshCharcoal = Color(0xFF0B0E14)       // app background
val FreshTextMuted = Color(0xFF9AA3B2)      // secondary text
val FreshTextFaint = Color(0xFF677180)

val FreshBackground = Color(0xFF0B0E14)     // near-black app background
val FreshSurface = Color(0xFF121622)        // cards / sheets
val FreshSurfaceAlt = Color(0xFF1F2637)     // wells / disabled chips
val FreshSurfaceHigh = Color(0xFF181D2B)    // elevated cards
val FreshOutline = Color(0xFF262E40)        // hairline borders

val FreshMuted = Color(0xFF9AA3B2)
val FreshAmber = Color(0xFFFFC043)          // break / cash accent
val FreshAmberDeep = Color(0xFFE8890C)
val FreshAmberContainer = Color(0xFF3A2C10)
val FreshError = Color(0xFFFF5A6A)          // vibrant error on dark
val FreshErrorContainer = Color(0xFF4A1216)
val FreshBlue = Color(0xFF6FA8FF)           // customer / info accent
val FreshPurple = Color(0xFFB197FF)
val FreshOrange = Color(0xFFFFA166)

/** Legacy light-first tokens (kept for reference / light mode). */
val LightBg = Color(0xFFF7F7F8)
val LightSurface = Color(0xFFFFFFFF)
val LightSurfaceAlt = Color(0xFFF1F1F2)
val LightOutline = Color(0xFFE8E8EB)
val LightText = Color(0xFF1A1A1A)
val LightTextMuted = Color(0xFF707070)
